...
def build_prompted_labels(
    ...
    # Whisper tokenizers use <|endoftext|> as bos_token_id while encoded labels
    # start with <|startoftranscript|>. The transcript start is always the first
    # encoded transcript token, so do not compare it with bos_token_id.
    mask_length = len(bounded_prompt_ids) + int(bool(transcript_label_ids))
