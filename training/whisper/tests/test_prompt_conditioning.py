...
class PromptConditioningTests(unittest.TestCase):
    ...
    def test_full_context_requires_explicit_opt_in(self) -> None:
        ...
        prompt = build_prompt_text({"context_before": "Topic: deploy\nTerms: Daiya"}, config)
    ...
    def test_unprompted_prompt_conditioning_row_still_shifts_transcript_start(self) -> None:
        prompted = build_prompted_labels(
            transcript_label_ids=[1, 10, 11, 12],
            prompt_token_ids=[],
            max_label_length=6,
        ...
        decoder_input_ids, labels = build_decoder_inputs_and_labels(
            prompted.labels,
            prompt_label_length=prompted.prompt_label_length,
            pad_token_id=0,
        )

        self.assertEqual(prompted.prompt_label_length, 1)
        self.assertEqual(decoder_input_ids, [1, 10, 11, 0])
        self.assertEqual(labels, [10, 11, 12, -100])
